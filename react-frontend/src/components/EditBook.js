...
export default function EditBook() {
  ...
  const fetchBookByName = async (name) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/v1/books/${name}`
      );
      ...
      const bookData = {
        ...
        authors: data.authors
          ? data.authors.map((author) => ({ name: author.name }))
          : [],
        description: data.description || "",
        categories: data.categories
          ? data.categories.map((category) => ({ name: category.name }))
          : [],
        image_url: data.image_url || "/images/blank.png",
      ...
      toast.error("Błąd podczas pobierania książki");
    }
  };
  ...
  const handleSearchSubmit = async (event) => {
    ...
    if (!searchBook) {
      toast.error("Podaj nazwę książki");
      return;
    }
    fetchBookByName(searchBook);
  };
  ...
  const handleUpdateSubmit = async (event) => {
    ...
    try {
      ...
      toast.error("Błąd podczas aktualizacji książki");
    }
  };
            ...
            <Card.Body>
              ...
              <Form onSubmit={handleUpdateSubmit}>
                <Form.Group controlId="formBookSearch">
                  <Form.Label>Wyszukaj książkę</Form.Label>
                  <Form.Control
                    required
                    type="text"
                ...
                {book && (
                  <div>
                    {/* Your AddBook form fields go here, but with pre-filled values */}
                    {/* Example: */}
